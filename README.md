...
5단계 프로세스로 정리한 스토리보드: ① 실시간 감시(다중 센서 데이터 자동 수집) → ② 이상 징후 탐지(위치·속도·열신호·SIGINT 융합 분석) → ③ AI 위협 분석(위협도 자동 평가, 근거 제시) → ④ 분석관 최종 판단(Human-in-the-Loop 승인/거부) → ⑤ 경보 및 보고(경보 발령 → Audit Log → SITREP 생성 → 상급 지휘부 보고). 전체 8단계 상세 버전과 이미지 생성 프롬프트는 `운영_시나리오_스토리보드.md` 참고.
