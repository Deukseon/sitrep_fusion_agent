...
실행: pytest test_track_history.py -v
...
TEST_HISTORY_PATH = os.path.join(tempfile.gettempdir(), "test_track_history.json")
...
@pytest.fixture
def accumulated_history() -> dict:
    """3사이클에 걸쳐 이력을 쌓은 결과를 다른 테스트(test_persistence)에서도 재사용할 수 있게 픽스처로 제공"""
...
def test_accumulation_and_recompute(accumulated_history):
...
def test_persistence(accumulated_history):
    print("\n=== 테스트 3: 파일 저장/로드 영속성 ===")
    history = accumulated_history
